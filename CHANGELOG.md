...
- **New alarm: `‼ integrity removed` — a same-version pin that loses its
  content hash.** Closes the last door on the v0.6.6/v0.6.8 bug class:
  formats whose checksum capture is strict-width (pubspec.lock,
  Podfile.lock, manifest.toml, rebar.lock, Manifest.toml) drop a
  *malformed* hash swap at the parser, and an npm/pnpm/yarn pin can have
  its `integrity` field deleted outright — both previously rendered as
  **"no changes"** while leaving the artifact completely unverified.
  A version pinned on both sides that carried a recognized content hash
  before and carries none now raises `‼ integrity removed` (counted
  under `-fail-on integrity`, SARIF rule `integrity-removed`). Two
  legitimate shapes stay silent: a dependency switched to a git/path
  override at the same version (the pin moved to a commit — registry
  hashes rightly vanish), and a whole file losing hashes at once (a
  lockfile regenerated by tooling that writes none). Validated against
  437 real lockfile history replays across nine formats (vscode, react,
  localsend ×2, fd, plausible/analytics, fresh, hugo go.sum, poetry,
  discourse) — zero false alarms, including localsend's three real
  hosted→git fork overrides, which suppress correctly.
