...
_lockvet() {
    ...
    case $prev in
        -C)
            ...
            return ;;
        -fail-on)
            COMPREPLY=( $(compgen -W "major vuln downgrade fresh deprecated license" -- "$cur") )
            return ;;
...
which packages are deprecated upstream; and which bumps change their
license.
...
\fBdowngrade\fR, \fBfresh\fR, \fBdeprecated\fR, \fBlicense\fR
(comma list).
.TP
.BI \-author " LIST"
