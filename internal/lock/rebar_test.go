...
func TestRebarLock(t *testing.T) {
	...
	// Each hash digests a different byte stream, so each carries its own
	// scope — see the parser comment.
	pin := f.Pin("certifi", "2.12.0")
	want := "contents#sha256:2d1cca2ec95f59643862af91f001478c9863c2ac9cb6e2f89780bfd8de987329" +
		" tarball#sha256:266da46bdb06d6c6bec50941d0d1fdb69edf4f345aa3e3bf6f14ce7e9c7ba2f7"
	if pin.Integrity != want {
		t.Errorf("certifi integrity = %q", pin.Integrity)
	}
	// redbug: inner hash only (no pkg_hash_ext entry).
	if got := f.Pin("redbug", "2.0.6").Integrity; got != "contents#sha256:63c8977a2f71f84a2acd7e7cfab74a5eaa787c110105379a34d63c562739b3cf" {
