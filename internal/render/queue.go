...
func (r QueueRow) verdict() int {
	...
	case r.NoChanges:
		return 3
	case r.Sum.VulnsIntroduced > 0 || r.Sum.Unlisted > 0 || r.Sum.ScriptsAdded > 0 || r.Sum.ProvenanceDropped > 0 || r.Sum.Typosquats > 0 || r.Sum.IntegrityChanged > 0 || r.Sum.IntegrityRemoved > 0 || r.Sum.TagMismatch > 0 || r.Sum.RegistryMoved > 0:
		return 0
...
func queueSummary(s styler, noun string, rows []QueueRow, vulnsChecked, metaChecked bool, freshDays int) string {
	...
	if vuln > 0 {
		parts = append(parts, s.bred(fmt.Sprintf("%d alarming (vulns/unlisted/typosquat/scripts/provenance/integrity)", vuln)))
	}
	if look > 0 {
