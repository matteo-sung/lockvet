...
func (r QueueRow) verdict() int {
	...
	case r.NoChanges:
		return 3
	case r.Sum.VulnsIntroduced > 0 || r.Sum.Unlisted > 0 || r.Sum.ScriptsAdded > 0:
		return 0
...
func queueSummary(s styler, noun string, rows []QueueRow, vulnsChecked, metaChecked bool, freshDays int) string {
	...
	if vuln > 0 {
		parts = append(parts, s.bred(fmt.Sprintf("%d alarming (vulns/unlisted/install scripts)", vuln)))
	}
	if look > 0 {
