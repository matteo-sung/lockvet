...
```sh
curl -fsSLO https://github.com/matteo-sung/lockvet/releases/download/v0.6.12/lockvet_v0.6.12_linux_amd64.deb
sudo dpkg -i lockvet_v0.6.12_linux_amd64.deb
```

Fedora / RHEL:

```sh
sudo rpm -i https://github.com/matteo-sung/lockvet/releases/download/v0.6.12/lockvet_v0.6.12_linux_amd64.rpm
```
...
```sh
curl -fsSLO https://github.com/matteo-sung/lockvet/releases/download/v0.6.12/lockvet_v0.6.12_linux_amd64.apk
apk add --allow-untrusted lockvet_v0.6.12_linux_amd64.apk
```
...
```sh
docker run --rm -v "$PWD:/repo" -w /repo ghcr.io/matteo-sung/lockvet:0.6.12 lockvet
```
...
```sh
gh attestation verify lockvet_v0.6.12_linux_amd64.tar.gz --owner matteo-sung
gh attestation verify oci://ghcr.io/matteo-sung/lockvet:0.6.12 --owner matteo-sung
```
...
jobs:
  triage:
    ...
    steps:
      - run: curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/main/install.sh | sh -s -- -b /usr/local/bin -v v0.6.12
...
jobs:
  audit:
    ...
    steps:
      ...
      - run: |
          curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/v0.6.12/install.sh | sh -s -- -b .
...
`{ "command": "docker", "args": ["run", "-i", "--rm", "ghcr.io/matteo-sung/lockvet:0.6.12", "lockvet", "mcp"] }`.
  ...
  lockvet:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      - uses: matteo-sung/lockvet@v0.6.12
        # optional:
        # with:
...
# .gitlab-ci.yml
lockvet:
  image: ghcr.io/matteo-sung/lockvet:0.6.12
  rules:
...
jobs:
  - job: lockvet
    ...
    container: ghcr.io/matteo-sung/lockvet:0.6.12
    steps:
      - checkout: none
      - script: >
          lockvet pr
