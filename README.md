...
```sh
curl -fsSLO https://github.com/matteo-sung/lockvet/releases/download/v0.6.14/lockvet_v0.6.14_linux_amd64.deb
sudo dpkg -i lockvet_v0.6.14_linux_amd64.deb
```

Fedora / RHEL:

```sh
sudo rpm -i https://github.com/matteo-sung/lockvet/releases/download/v0.6.14/lockvet_v0.6.14_linux_amd64.rpm
```
...
```sh
curl -fsSLO https://github.com/matteo-sung/lockvet/releases/download/v0.6.14/lockvet_v0.6.14_linux_amd64.apk
apk add --allow-untrusted lockvet_v0.6.14_linux_amd64.apk
```
...
```sh
docker run --rm -v "$PWD:/repo" -w /repo ghcr.io/matteo-sung/lockvet:0.6.14 lockvet
```
...
```sh
gh attestation verify lockvet_v0.6.14_linux_amd64.tar.gz --owner matteo-sung
gh attestation verify oci://ghcr.io/matteo-sung/lockvet:0.6.14 --owner matteo-sung
```
...
jobs:
  triage:
    ...
    steps:
      - run: curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/main/install.sh | sh -s -- -b /usr/local/bin -v v0.6.14
...
jobs:
  audit:
    ...
    steps:
      ...
      - run: |
          curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/v0.6.14/install.sh | sh -s -- -b .
...
`{ "command": "docker", "args": ["run", "-i", "--rm", "ghcr.io/matteo-sung/lockvet:0.6.14", "lockvet", "mcp"] }`.
  ...
  lockvet:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      - uses: matteo-sung/lockvet@v0.6.14
        # optional:
        # with:
...
# .gitlab-ci.yml
lockvet:
  image: ghcr.io/matteo-sung/lockvet:0.6.14
  rules:
...
jobs:
  - job: lockvet
    ...
    container: ghcr.io/matteo-sung/lockvet:0.6.14
    steps:
      - checkout: none
      - script: >
          lockvet pr
...
- **`orbs:` pins** — reusable CI packages from the [CircleCI orb
  registry](https://circleci.com/developer/orbs), whose commands run in
...
| Erlang | `rebar.lock` — Hex advisories, ages/retirements/unlisted from hex.pm, direct deps from the lock's own level numbers, `pkg_hash`/`pkg_hash_ext` checksums as integrity pins each scoped on its own — a same-version change of *either* hash surfaces as ‼ repinned, even with the other left intact; renamed forks (`{<<"uuid">>,{pkg,<<"uuid_erl">>,…}}`) resolve under their real Hex package name; git/path deps exempt |
