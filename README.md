...
- **across every ecosystem, in one static binary** — 43 formats:
  ...
  overrides and `helmCharts:` entries), **pre-commit hook pins**
...
```sh
curl -fsSLO https://github.com/matteo-sung/lockvet/releases/download/v0.5.17/lockvet_v0.5.17_linux_amd64.deb
sudo dpkg -i lockvet_v0.5.17_linux_amd64.deb
```

Fedora / RHEL:

```sh
sudo rpm -i https://github.com/matteo-sung/lockvet/releases/download/v0.5.17/lockvet_v0.5.17_linux_amd64.rpm
```
...
```sh
curl -fsSLO https://github.com/matteo-sung/lockvet/releases/download/v0.5.17/lockvet_v0.5.17_linux_amd64.apk
apk add --allow-untrusted lockvet_v0.5.17_linux_amd64.apk
```
...
```sh
docker run --rm -v "$PWD:/repo" -w /repo ghcr.io/matteo-sung/lockvet:0.5.17 lockvet
```
...
```sh
gh attestation verify lockvet_v0.5.17_linux_amd64.tar.gz --owner matteo-sung
gh attestation verify oci://ghcr.io/matteo-sung/lockvet:0.5.17 --owner matteo-sung
```
...
jobs:
  triage:
    ...
    steps:
      - run: curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/main/install.sh | sh -s -- -b /usr/local/bin -v v0.5.17
...
jobs:
  audit:
    ...
    steps:
      ...
      - run: |
          curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/v0.5.17/install.sh | sh -s -- -b .
...
`{ "command": "docker", "args": ["run", "-i", "--rm", "ghcr.io/matteo-sung/lockvet:0.5.17", "lockvet", "mcp"] }`.
  ...
  lockvet:
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          fetch-depth: 0
      - uses: matteo-sung/lockvet@v0.5.17
        # optional:
        # with:
...
# .gitlab-ci.yml
lockvet:
  image: ghcr.io/matteo-sung/lockvet:0.5.17
  rules:
...
jobs:
  - job: lockvet
    ...
    container: ghcr.io/matteo-sung/lockvet:0.5.17
    steps:
      - checkout: none
      - script: >
          lockvet pr
...
public.ecr.aws, registry.gitlab.com). Images from any other host — private
...
`clusters/`, `gitops/`), files named `*.k8s.yaml`, and the conventional
workload basenames (`deployment.yaml`, `statefulset.yaml`, `daemonset.yaml`,
`replicaset.yaml`, `cronjob.yaml`, `pod.yaml`) — strictly: a document must
...
| Kubernetes | manifests (`*.yaml` under conventional directories — `k8s/`, `kubernetes/`, `manifests/`, `deploy/`, `overlays/`, `base/`, `clusters/`, `gitops/` — plus `*.k8s.yaml` and conventional basenames like `deployment.yaml` — top-level `apiVersion:` + `kind:` required, Helm `templates/` excluded): every `image:` under `containers:` / `initContainers:` / `ephemeralContainers`, verified like Dockerfile images. `kustomization.yaml` / `kustomization.yml`: `images:` transformer pins (`newTag:`, `digest:`) plus `helmCharts:` entries checked against the chart repository's own `index.yaml` — see [Kubernetes manifests & kustomizations](#kubernetes-manifests--kustomizations) |
