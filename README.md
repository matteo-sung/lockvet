...
jobs:
  triage:
    ...
    steps:
      - run: curl -fsSL https://raw.githubusercontent.com/matteo-sung/lockvet/main/install.sh | sh -s -- -b /usr/local/bin -v v0.3.3
...
`{ "command": "docker", "args": ["run", "-i", "--rm", "ghcr.io/matteo-sung/lockvet:0.3.3", "lockvet", "mcp"] }`.
...
# .gitlab-ci.yml
lockvet:
  image: ghcr.io/matteo-sung/lockvet:0.3.3
  rules:
...
jobs:
  - job: lockvet
    ...
    container: ghcr.io/matteo-sung/lockvet:0.3.3
    steps:
      - checkout: none
      - script: >
          lockvet pr
