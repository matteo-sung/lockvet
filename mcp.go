...
version is, what's deprecated, what's missing from the registry index, what suddenly runs install scripts, what silently drops sigstore provenance
	...
	return []map[string]any{
		{
			...
			"description": "Explain the lockfile/dependency changes of a pull/merge request, compare range, or commit on GitHub, GitLab, Bitbucket Cloud, Gitea/Forgejo (incl. Codeberg), or Azure DevOps — no clone needed. " +
				"Reports every added/removed/bumped package with semver severity, vulnerabilities introduced/fixed/unresolved (OSV.dev), release age, deprecations, license changes, unlisted versions, npm install-script and npm/PyPI/crates.io/RubyGems provenance-dropped transitions (ages/deprecations cover Packagist, hex.pm, pub.dev, jsr.io and CocoaPods too), direct-vs-transitive origin with via-chains, and verified upstream changelog/diff links. " +
		...
		},
		{
			...
			"description": "Explain the dependency changes between two files on disk, no git needed: two lockfiles (any of the 29 supported formats; one side may carry a suffix, e.g. Cargo.lock.orig vs Cargo.lock) or two CycloneDX/SPDX JSON SBOMs under any filename — e.g. syft scans of two container images.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
