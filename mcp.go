...
(unpublished/pulled releases), and license changes — across 61 lockfile
formats (npm, pnpm, yarn, bun, Cargo, uv, poetry, pdm.lock, go.mod, composer, Gemfile,
conda, Julia, Haskell, Scala/sbt, Terraform, Helm, Ansible Galaxy, vcpkg, renv, SBOMs, and more).
...
func mcpTools() []map[string]any {
	...
	return []map[string]any{
		...
		},
		{
			...
			"description": "Explain the dependency changes between two files on disk, no git needed: two lockfiles (any of the 61 supported formats; one side may carry a suffix, e.g. Cargo.lock.orig vs Cargo.lock) or two CycloneDX/SPDX JSON SBOMs under any filename — e.g. syft scans of two container images.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
