...
(unpublished/pulled releases), and license changes — across 39 lockfile
	...
	return []map[string]any{
		{
			...
			"description": "Explain the lockfile/dependency changes of a pull/merge request, compare range, or commit on GitHub, GitLab, Bitbucket Cloud, Gitea/Forgejo (incl. Codeberg), or Azure DevOps — no clone needed. " +
				"Reports every added/removed/bumped package with semver severity, vulnerabilities introduced/fixed/unresolved (OSV.dev), release age, deprecations, license changes, unlisted versions, typosquat-suspect additions (new young packages one edit from a popular name), integrity changes on unchanged versions, SwiftPM/workflow pins verified against upstream tags (moved-tag and not-a-release catches) and private-to-public registry resolution moves (dependency-confusion shape), npm install-script and npm/PyPI/crates.io/RubyGems provenance-dropped transitions (ages/deprecations cover Packagist, hex.pm, pub.dev, jsr.io, CocoaPods and CRAN too), direct-vs-transitive origin with via-chains, and verified upstream changelog/diff links. " +
		...
		},
		{
			...
			"description": "Explain the dependency changes between two files on disk, no git needed: two lockfiles (any of the 38 supported formats; one side may carry a suffix, e.g. Cargo.lock.orig vs Cargo.lock) or two CycloneDX/SPDX JSON SBOMs under any filename — e.g. syft scans of two container images.",
			"inputSchema": map[string]any{
				"type": "object",
				"properties": map[string]any{
